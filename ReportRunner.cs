...
namespace fyiReporting.RdlCmd
{
    class ReportRunner
    {
        ...
        //  static private bool _saveLocal = false;


        public void setStampInfo(string value)
        {
            _StampInfo = value;

        }
        public void setDataSourcePW(string value)
        {
        ...
        public void DoRender(string dir, string[] files,string[] types , string ward, string startdate, string enddate)
        {
            string source;
            Report report;
            int index;
            ...
            // Console.WriteLine("filename string {0}", files);
            foreach (string filename in files)
            {
                 ...
                 * 
                 * c:\work\RdlCmd\bin\Debug>rdlcmd /fC:\Users\Auditor\Documents\work\Legal-AnnualAccountingRevised_2015.rdl?rc:ofile=5515 -tpdf  -oC:\work  -pmerlin -w5515 
                 * -s01/01/2014 -e12/31/2014
                 * ************************************************************************** */
                ld = new ListDictionary();
                ld.Add("Ward", ward);
                ld.Add("rc:ofile", ward + "_temporary");
                ld.Add("StartDate", startdate);
                ld.Add("EndDate", enddate);
        ...
        private void SaveAsPdf(Report report, OneFileStreamGen sg)
        {
            ...
            foreach (fyiReporting.RDL.Page p in pgs)		// we loop then break after obtaining one
            {
                try
                {
        ...
        static public void WriteHelp()
        {
            ...
            Console.WriteLine("-w the ward number whos report we want to produce");
            Console.WriteLine("-e Report End Date");
            Console.WriteLine("-s Reporting Start Date");
            Console.WriteLine("/? outputs this text");
        }

        static public void WriteLicense()
        {
